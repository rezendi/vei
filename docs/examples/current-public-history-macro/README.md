...
vei whatif benchmark news-state-point \
  ...
  --label mixed_macro_20260516 \
  --topic all_public_record \
  --as-of 2026-05-16 \
  --candidate "customer_status_note::Publish inflation-risk bulletin::Publish a dated public bulletin that separates inflation, energy, labor-market, and Treasury-yield signals visible by 2026-05-16." \
...
The fixture records exact source fetches in `workspace/source_manifest.json`. The latest daily prediction/result comparison and champion/challenger decision are saved in `workspace/current_macro_state_point_result.json`.
